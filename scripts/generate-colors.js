 ...
 * HEX color variables defined in the :root and .dark:root selectors,
...
// eslint-disable-next-line no-undef
const colorsTsPath = path.join(__dirname, '../constants/generatedColors.ts')
...
// Extract HEX color variables from both :root and .dark:root
...
const extractColors = (cssBlock) => {
	// Match hex colors, including alpha (e.g. #FFFFFF, #FFFFFFAA)
	const hexRegex = /--([\w-]+):\s*(#[0-9A-Fa-f]{6,8});/g
	const colors = {}
	let match

	while ((match = hexRegex.exec(cssBlock)) !== null) {
		// eslint-disable-next-line no-unused-vars
		const [_, name, hex] = match
		colors[toCamelCase(name)] = hex.toUpperCase()
	}
...
console.log(`✅ generatedColors.ts has been generated at ${colorsTsPath}`)
